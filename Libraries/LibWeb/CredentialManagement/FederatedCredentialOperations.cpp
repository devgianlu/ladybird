...
WebIDL::ExceptionOr<GC::Ref<FederatedCredential>> create_federated_credential(JS::Realm& realm, FederatedCredentialInit const& init, Optional<URL::Origin> origin)
{
    ...
    // 4. Return c.
    return realm.create<FederatedCredential>(realm, init, move(origin.value()));
}
