...
JS::ThrowCompletionOr<Variant<Empty, GC::Ref<Credential>, GC::Ref<CreateCredentialAlgorithm>>> PublicKeyCredentialInterface::create(JS::Realm& realm, URL::Origin const& origin, CredentialCreationOptions const& options, bool same_origin_with_ancestors) const
{
    ...
    if (!effective_domain.has_value())
        return throw_completion(WebIDL::SecurityError::create(realm, "Caller origin must have a valid domain"_string));
    ...
    if (pk_options.rp.id.has_value()) {
        ...
        // does not support related origin requests
            ...
            return throw_completion(WebIDL::SecurityError::create(realm, "rp.id must be a registrable domain suffix of the caller origin"_string));
        }
    }
    //    is not present
    else {
    ...
    // is non-zero
        ...
        if (cred_types_and_pub_key_algs.is_empty())
            return throw_completion(WebIDL::NotSupportedError::create(realm, "No supported credential types"_string));
    }
    ...
    return throw_completion(WebIDL::NotAllowedError::create(realm, "????"_string));
}
