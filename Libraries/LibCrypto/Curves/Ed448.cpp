...
ErrorOr<ByteBuffer> Ed448::generate_private_key()
{
    ...
    ArmedScopeGuard free_key { [&] { EVP_PKEY_free(key); } };

    size_t key_size = EVP_PKEY_get_size(key);
    auto buf = TRY(ByteBuffer::create_uninitialized(key_size));

    OPENSSL_TRY(EVP_PKEY_get_raw_private_key(key, buf.data(), &key_size));
...
{
    auto* key = OPENSSL_TRY_PTR(EVP_PKEY_new_raw_private_key(EVP_PKEY_ED448, nullptr, private_key.data(), private_key.size()));
    ArmedScopeGuard free_key { [&] { EVP_PKEY_free(key); } };

    size_t key_size = EVP_PKEY_get_size(key);
    auto buf = TRY(ByteBuffer::create_uninitialized(key_size));

    OPENSSL_TRY(EVP_PKEY_get_raw_public_key(key, buf.data(), &key_size));
...
{
    auto* key = OPENSSL_TRY_PTR(EVP_PKEY_new_raw_private_key_ex(nullptr, "ED448", nullptr, private_key.data(), private_key.size()));
    ArmedScopeGuard free_key { [&] { EVP_PKEY_free(key); } };

    auto* ctx = EVP_MD_CTX_new();
    ArmedScopeGuard free_ctx { [&] { EVP_MD_CTX_free(ctx); } };
    ...
    OPENSSL_TRY(EVP_DigestSignInit_ex(ctx, nullptr, nullptr, nullptr, nullptr, key, params));

    size_t sig_len = signature_size();
    auto sig = TRY(ByteBuffer::create_uninitialized(sig_len));

    OPENSSL_TRY(EVP_DigestSign(ctx, sig.data(), &sig_len, message.data(), message.size()));
...
ErrorOr<bool> Ed448::verify(ReadonlyBytes public_key, ReadonlyBytes signature, ReadonlyBytes message, ReadonlyBytes context)
{
    ...
    OPENSSL_TRY(EVP_DigestVerifyInit_ex(ctx, nullptr, nullptr, nullptr, nullptr, key, params));

    auto res = EVP_DigestVerify(ctx, signature.data(), signature.size(), message.data(), message.size());
    if (res == 1)
        return true;
    if (res == 0)
        return false;
